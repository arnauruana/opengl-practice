...
void Scene::init()
{
	...
	texs[0].loadFromFile("rsc/media/img/sky.jpg", TEXTURE_PIXEL_FORMAT_RGBA);
	texs[1].loadFromFile("rsc/media/img/bricks.jpg", TEXTURE_PIXEL_FORMAT_RGBA);
	texs[2].loadFromFile("rsc/media/img/mario.png", TEXTURE_PIXEL_FORMAT_RGBA);
	texs[3].loadFromFile("rsc/media/img/jump.png", TEXTURE_PIXEL_FORMAT_RGBA);
	texs[4].loadFromFile("rsc/media/img/cloud.png", TEXTURE_PIXEL_FORMAT_RGBA);
	...
	if (not text.init("rsc/font/OpenSans-Bold.ttf"))
	{
...
void Scene::render()
{
	...
	if (this->textP)
	{
		text.render("Arnau Ruana - VJ", glm::vec2(5, glutGet(GLUT_WINDOW_HEIGHT) - 5), 24, glm::vec4(1, 1, 1, 1));
	}
}
...
void Scene::initShaders()
{
	...
	vShader.initFromFile(VERTEX_SHADER, "rsc/shader/simple.vert");
	if (!vShader.isCompiled())
	{
		cout << "Vertex Shader Error" << endl;
		cout << "" << vShader.log() << endl << endl;
	}
	fShader.initFromFile(FRAGMENT_SHADER, "rsc/shader/simple.frag");
	...
	vShader.initFromFile(VERTEX_SHADER, "rsc/shader/texture.vert");
	if (!vShader.isCompiled())
	{
		cout << "Vertex Shader Error" << endl;
		cout << "" << vShader.log() << endl << endl;
	}
	fShader.initFromFile(FRAGMENT_SHADER, "rsc/shader/texture.frag");
	if (!fShader.isCompiled())
	{
		cout << "Fragment Shader Error" << endl;
		cout << "" << fShader.log() << endl << endl;
